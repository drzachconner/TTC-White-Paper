# Requirements: TTC White Paper Editorial Production

**Defined:** 2026-02-26
**Core Value:** Credible, voice-preserving white paper ready for professional distribution

## v1 Requirements

### Citations

- [x] **CITE-01**: All references verified with correct authors, years, volumes, pages
- [x] **CITE-02**: DOIs added to all verifiable journal articles
- [x] **CITE-03**: Vague Epstein/Senzon/Ward citations replaced with specific publications
- [x] **CITE-04**: All in-text citations match References entries

### Structure & Redundancy

- [x] **STRC-01**: Executive Summary leads with clinical mechanism, then philosophical frame
- [x] **STRC-02**: "Misinformation / missing information" framework appears in one primary location with brief cross-references elsewhere
- [x] **STRC-03**: "We do not seek to apply force to what is stuck" has one definitive placement
- [x] **STRC-04**: Cascade sequence (dyskinesia, dysafferentation, etc.) consolidated between Parts III and IV
- [x] **STRC-05**: Valuable content lost from earlier renditions recovered
- [x] **STRC-06**: Unincorporated Nov 2025 editorial feedback applied

### Credibility & Claims

- [ ] **CRED-01**: Every factual/clinical claim reviewed for evidence strength match
- [ ] **CRED-02**: Intent section (PEAR/RNG) treatment decided and implemented (keep/reframe/appendix)
- [ ] **CRED-03**: Evidence Map section honestly distinguishes established vs emerging vs speculative
- [ ] **CRED-04**: No overclaiming — citation strength matches claim strength

### Voice & Polish

- [ ] **VOIC-01**: Register harmonized — clinical-scientific primary, philosophical as intentional seasoning
- [ ] **VOIC-02**: AI-sounding phrasing eliminated throughout
- [ ] **VOIC-03**: Consistent terminology (pick one term where variants exist)
- [ ] **VOIC-04**: Prose tightened — filler removed, transitions sharpened

### Adversarial Review

- [ ] **ADVR-01**: External LLM (GPT-5.2) adversarial critique received
- [ ] **ADVR-02**: All valid critique points addressed in final draft

### Final Delivery

- [ ] **DELV-01**: Final polished markdown committed to repo
- [ ] **DELV-02**: PDF exported and ready for distribution
- [ ] **DELV-03**: Vital aspects checklist verified — all key TTC principles survive edits

## v2 Requirements

### Enhanced Distribution

- **DIST-01**: Formatted for specific seminar handout layout
- **DIST-02**: Companion slide deck summarizing key points
- **DIST-03**: One-page executive summary as standalone document

### Academic Adaptation

- **ACAD-01**: Journal-formatted version with structured abstract
- **ACAD-02**: Supplementary materials appendix for journal submission

## Out of Scope

| Feature | Reason |
|---------|--------|
| Journal submission formatting | White paper format chosen deliberately |
| New original research / data | Editorial refinement only |
| Rewriting from scratch | Refine existing voice, not replace it |
| Graphic design / layout | Content-only project; design is separate |
| New sections or major additions | Consolidate and polish what exists |

## Traceability

| Requirement | Phase | Status |
|-------------|-------|--------|
| CITE-01 | Phase 1 | Complete |
| CITE-02 | Phase 1 | Complete |
| CITE-03 | Phase 1 | Complete |
| CITE-04 | Phase 1 | Complete |
| STRC-01 | Phase 2 | Complete |
| STRC-02 | Phase 2 | Complete |
| STRC-03 | Phase 2 | Complete |
| STRC-04 | Phase 2 | Complete |
| STRC-05 | Phase 2 | Complete |
| STRC-06 | Phase 2 | Complete |
| CRED-01 | Phase 3 | Pending |
| CRED-02 | Phase 3 | Pending |
| CRED-03 | Phase 3 | Pending |
| CRED-04 | Phase 3 | Pending |
| VOIC-01 | Phase 4 | Pending |
| VOIC-02 | Phase 4 | Pending |
| VOIC-03 | Phase 4 | Pending |
| VOIC-04 | Phase 4 | Pending |
| ADVR-01 | Phase 5 | Pending |
| ADVR-02 | Phase 5 | Pending |
| DELV-01 | Phase 6 | Pending |
| DELV-02 | Phase 6 | Pending |
| DELV-03 | Phase 6 | Pending |

**Coverage:**
- v1 requirements: 23 total
- Mapped to phases: 23
- Unmapped: 0

---
*Requirements defined: 2026-02-26*
*Last updated: 2026-02-26 after initial definition*
